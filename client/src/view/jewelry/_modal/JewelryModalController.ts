...
export const JewelryModalController: SignalType<any, any> =
    new SignalController({
        ...
        aiPhotosRaw: [] as File[],
        aiPhotos: [] as string[],
        aiPhotosSelected: {} as Record<number, boolean>,
        aiPhotosLoading: false
    }, {
        ...
        addAiPhotoRaw: function (this: any, files: File[]) {
            const existingNames = new Set(this.aiPhotosRaw.map((f: File) => f.name));
            const newFiles = files.filter((f: File) => !existingNames.has(f.name));
            this.aiPhotosRaw = [...this.aiPhotosRaw, ...newFiles];
        },
        removeAiPhotoRaw: function (this: any, file: File) {
            this.aiPhotosRaw = this.aiPhotosRaw.filter((f: File) => f !== file);
        },
        generateAiPhotos: async function (this: any) {
            this.aiPhotosLoading = true;
            try {
                const uploaded = await uploadToFirebaseStorage(this.aiPhotosRaw, 'ai-raw');
                const rawUrls = uploaded.map(({url}) => url);
                const response = await FetchApi(
                    'v1/ai/product_photos',
                    'GET',
                    null,
                    {image_urls: rawUrls},
                );
                if (response.success) {
                    this.aiPhotos = [...this.aiPhotos, ...response.data];
                }
            } catch (error) {
                console.error('AI photo generation failed:', error);
                window.toast.E('Failed to generate AI photos.');
            }
            this.aiPhotosLoading = false;
        },
        toggleAiPhotoSelection: function (this: any, index: number) {
            this.aiPhotosSelected = {
                ...this.aiPhotosSelected,
                [index]: !this.aiPhotosSelected[index],
            };
        },
        saveSelectedAiPhotos: async function (this: any, productId: number) {
            const selected: string[] = [];
            const rejected: string[] = [];
            this.aiPhotos.forEach((url: string, i: number) => {
                if (this.aiPhotosSelected[i]) {
                    selected.push(url);
                } else {
                    rejected.push(url);
                }
            ...
            if (!selected.length) {
                return window.toast.W('No photos selected.');
            }

            const {product_image_url} = this.formData.product;
            const newImages = selected.map((url) => ({
                document_primary: false,
                document_type: 'image',
                document_url: url,
            }));
            const updatedImages = [...product_image_url, ...newImages];

            await FetchApi('v1/product', 'PUT', {
                product_id: productId,
                product_image_url: updatedImages,
            });

            if (rejected.length) {
                await deleteFromFirebaseStorage(rejected);
            }

            // Refresh modal data
            await this.editMap.product(this.fields, productId);

            // Clear AI photos state
            this.aiPhotos = [];
            this.aiPhotosRaw = [];
            this.aiPhotosSelected = {};
            window.toast.S('AI photos saved.');
        },
